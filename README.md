...
Full-waveform inversion (FWI) reconstructs a subsurface velocity model $x$ from seismic recordings $y$. For source $s$, the acoustic wavefield satisfies
...
where $q_s$ is the source wavelet. Sampling the wavefield at receiver locations defines the forward operator $H$:
...
The frozen Flow Map provides the prior over $x$; Multi-Back minimizes waveform mismatch through $H$ without retraining that prior.

![Velocity model x and its five-source observed seismic data y](assets/figures/fwi_xy_example.png)

*A paired example from the data-generation pipeline: $x\in\mathbb R^{70\times70}$ is the velocity model and $y=H(x)\in\mathbb R^{5\times150\times70}$ is the seismic observation.*

Each entry is

```math
y_{s,t,r}=[H(x)]_{s,t,r}=u_s(\mathbf r_r,t),
```

The dimensions of $y$ are:

- **5 sources:** one source gather per panel;
- **150 time steps:** the vertical axis of each gather;
- **70 receivers:** the horizontal axis of each gather.

The strong V-shaped events are direct arrivals; weaker later events are reflections from interfaces in $x$. Red and blue denote signed waveform amplitude. FWI adjusts $x$ until $H(x)$ matches these observations. The repository's **public-$H$ key** is a residual score computed from $H(x)$ and $y$, not the forward operator itself.
