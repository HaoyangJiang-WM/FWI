...
The observation is a tensor $y\in\mathbb R^{5\times150\times70}$ with entries

```math
y_{s,t,r}=[H(x)]_{s,t,r}=u_s(\mathbf r_r,t),
```

where $u_s$ is the wavefield generated by source $s$ in velocity model $x$, and $\mathbf r_r$ is receiver $r$'s location. The three axes explain the five panels:

- **5 sources:** each panel fixes one source index $s$. The source is fired from a different horizontal location, so the direct-arrival pattern shifts across panels.
- **150 time steps:** each panel's vertical axis indexes the recorded wavefield from early time at the top to late time at the bottom. This is the discretized $t$ in the acoustic wave equation.
- **70 receivers:** each panel's horizontal axis indexes 70 receiver locations. For a fixed source and time, one row shows the amplitudes measured simultaneously by all receivers.

The strong V-shaped or diagonal event is the direct wave traveling from the active source to the receiver array. The weaker later events are reflections and transmissions produced by interfaces in $x$. Red and blue indicate opposite signed wave amplitudes, not high and low velocity. FWI adjusts $x$ so that the simulated tensor $H(x)$ matches all $5\times150\times70$ observed samples in $y$.
